...
### 📄 Документация — финальные оценки 23.09 (Kimi 8/10, DeepSeek 8.5/10, Qwen 8.5/10)

- **«Оценка kimi/дипсик/qwen.rtf»** изучены, сводный остаток интегрирован в корневой `Что-доделать.txt`
  (раздел «Финальные оценки 23.09») и синхронизирован в `test-analysis/`
- **План этапов расширен** 54–60 → **54–61**: Этап 57 + Race-тесты, Этап 58 + a11y, Этап 59 + auth/oidc.test.js
  и background sub-тесты, Этап 60 + i18n parity/contract/migration-idempotency, новый Этап 61 — покрытие ядра + E2E-реализм
- **`Промты.txt`**: добавлены готовые промты A–H под остаток (race, oidc, ticket-context.optimistic, E2E-real, i18n/XSS, a11y, background, contract/миграции) + список файлов для re-аудита Этапа 60
- **`Тесты дипсик.txt` / `Тесты qwen.txt`**: проставлен статус-баннер «что уже закрыто Этапами 46–56 / что осталось»
- **`AGENTS.md`**: новое правило 12 «фича = флаг» (модуль без feature flag не мержится); обновлены числа тестов (791 серверных) и check-console (17 страниц)
- **`context.txt`**: E2E 16/51 → **19/57** (актуализация)
