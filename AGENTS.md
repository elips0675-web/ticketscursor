...
11. **В начале каждой задачи** — читать корневой `Что-доделать.txt` (синхронизирован с `test-analysis/Что-доделать.txt`): брать следующий незакрытый этап из «ПЛАН ДОРАБОТКИ ПО ЭТАПАМ (54–67)», статус отмечать в этом же файле. После завершения этапа — обновить «Что сделано.txt» и синхронизировать копию `test-analysis/Что-доделать.txt` (если правил корневую) или наоборот
...
13. **DoD на фичу** (аудит 24.09, «Оценка ии.rtf» №12): фича не считается готовой, пока выполнены все 5 пунктов — флаг (`useFeature()`), тест, i18n (оба языка), `check-console.mjs` (errors: [], hasRussianText: true), актуальный инвентарь (`test-analysis/test-inventory.json` + `*‑tests.txt`)
14. **«AI-пункт без цитаты кода не идёт в план»** (аудит 24.09): пункт из аудита попадает в «Что-доделать.txt» только вместе с конкретной ссылкой на файл/функцию/тест — иначе остаётся в бэклоге-списке
15. **«Сначала фикс, потом тест»** (аудит 24.09): тест, фиксирующий баг, пишется после устранения причины (сначала исправить код, потом закрепить тестом); тест на ещё-неисправленный баг не коммитится
16. **Пороги coverage — буфер +2 п.п.** (аудит 24.09): пороги в `vitest.config.js`/`vitest.client.config.ts` держать на 2 п.п. ниже фактического покрытия; после роста покрытия — поднять пороги, после падения — сначала поднять покрытие, потом пороги
...
| | `vitest run` | Юнит-тесты (520 клиентских + 808 серверных) |
...
### Этапы 32–63 — см. «Что сделано.txt» (детали каждого этапа — там)
- Актуальный инвентарь на 25.09.2026: клиент **70 файлов / 625 тестов**, сервер **71 файл / 947 тестов** = **1572 теста, 0 failures**, E2E 19 spec'ов / 61 тест
...
- **Этап 63 (подфича 1) — 2FA/TOTP**: `server/src/auth/totp.js` (otplib v12: generateSecret/verifySync/generateURI), `server/src/feature-flags.js` (кэш 30с + invalidateFeatureFlagCache); миграции `20260925_user_totp.js` + `20260925_two_fa_flag.js` (флаг `two_fa`, дефолт **off**); двухшаговый логин — `step:'2fa'` + `tempToken` (JWT purpose '2fa', 5 мин) при `user_totp.enabled`, `require2faSetup` (admin + флаг); `POST /2fa/verify`, `GET /2fa/status`, `POST /2fa/setup|enable|disable` (`requireRole('admin')`); require2FA middleware в `admin.js` (403 `2FA_REQUIRED`, исключение `/features`); QR-код (`qrcode`) + `TwoFactorSection.tsx` в Profile под `useFeature('two_fa')`, экран ввода кода в Login.tsx; i18n ru/en (`auth.twoFactor*`/`profile.twoFactor*`); тесты `auth-2fa.test.js` (14 — изоляция `vi.mock` флага + выделенный админ id 600, устранена гонка login с api.test.js), `TwoFactorSection.test.tsx` (5), Login.expanded 11→13; 🐞 фикс `requireRole('admin','super_admin')` → `requireRole('admin')` (Math.max ролей требовал super_admin); пороги клиент 70/61/63/73, сервер 72/66/73/73 (факт − 2п.п.)
- **Этап 63 (подфича 2) — Активные сессии**: миграция `20260925_user_sessions.js` (idempotent: колонки device_name/ip_address/user_agent/last_seen_at в `refresh_tokens` + флаг `user_sessions`, дефолт **off**); `server/src/auth/session-meta.js` (`parseUserAgent` → «Браузер · ОС», isMobile; `sessionMeta(req)` — ip из x-forwarded-for || remoteAddress); `storeRefreshToken(userId, token, familyId, meta)` в login/2fa/refresh; `GET /auth/sessions` (список своих, token не утекает, current — сверка cookie `refreshToken`), `POST /auth/sessions/:id/revoke` (своя ok / чужая 404 / текущая 400 / «abc» 400); UI `SessionsSection.tsx` в Profile под `useFeature('user_sessions')` (Badge «Текущая сессия», revoke, «Завершить все сессии» → `POST /auth/revoke-all`); i18n `profile.sessions*` (10); тесты `auth-sessions.test.js` (10 — изоляция как у 2FA, выделенные id 601/602), `SessionsSection.test.tsx` (6); пороги клиент 71/61/63/73 (факт 73.08/63.99/65.33/75.41), сервер 72/66/73/73 (факт 74.78/68.82/75.98/75.75) (факт − 2п.п.)
- **Этап 63 (подфича 3) — Настройки уведомлений**: миграция `20260925_notification_prefs.js` (idempotent: таблица `notification_preferences` user_id UNIQUE + prefs JSON + флаг `notification_prefs`, дефолт **off**); `server/src/notification-prefs.js` (NOTIF_EVENTS 8 событий × NOTIF_CHANNELS email/push/in_app, `normalizePrefs` → `{valid, prefs}` + дефолты все включены, кэш 30с, `allowedUserIds`); `GET/PUT /notifications/preferences` (API **вне флага**, upsert, невалидный payload → 400); notify.js — все 7 notify-функций фильтруют каналы через `allowedUserIds`, push стал реальным (`sendPushToUsers` через web-push, fire-and-forget, 404/410 → cleanup подписки; фикс `emailTargets.includes` → `emailIds.has`); UI `NotificationPreferencesSection.tsx` в Profile под `useFeature('notification_prefs')` (таблица 8×3, мастер-чекбоксы, Save→PUT); i18n `profile.prefs.*`; тесты `notification-prefs.test.js` (11 — изоляция id 603/604), notify.test.js 17→22, `NotificationPreferencesSection.test.tsx` (6); пороги клиент 71/62/63/73 (факт 73.48/64.19/65.78/75.79), сервер 73/67/74/74 (факт 75.03/69.15/76.51/76.05) (факт − 2п.п.)
